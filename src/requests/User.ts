 ...
 * @public
 */
export function affiliates(id: string, count?: number, cursor?: string): AxiosRequestConfig {
	return {
		method: 'get',
 ...
 * @public
 */
export function bookmarks(count?: number, cursor?: string): AxiosRequestConfig {
	return {
		method: 'get',
...
/**
 * @param count - The number of tweets in notification to fetch.
 * @param cursor - The cursor to the batch of tweets in notification to fetch.
 *
 * @public
 */
export function tweetsInNotification(count?: number, cursor?: string): AxiosRequestConfig {
	return {
		method: 'get',
		url: 'https://x.com/i/api/2/notifications/device_follow.json',
		params: {
			/* eslint-disable @typescript-eslint/naming-convention */
			include_profile_interstitial_type: 1,
			include_blocking: 1,
			...
			count: count,
			cursor: cursor,
			ext: 'mediaStats%2ChighlightedLabel%2CparodyCommentaryFanLabel%2CvoiceInfo%2CbirdwatchPivot%2CsuperFollowMetadata%2CunmentionInfo%2CeditControl%2Carticle',
			/* eslint-enable @typescript-eslint/naming-convention */
		},
	};
}
